...
import com.huawei.codelab.data.AdvertisementMo;
import com.huawei.codelab.data.VideoListMo;
import com.huawei.codelab.data.VideoTabStyle;
import com.huawei.codelab.provider.AdvertisementProvider;
...
import com.huawei.codelab.util.MediaUtil;
...
public class MainAbilitySlice extends AbilitySlice {
    ...
    private void initList() {
        ...
        commonProvider = new CommonProvider<VideoListMo>(MediaUtil.getPlayListMos(), getContext(),
                ResourceTable.Layout_recommend_gv_item) {
            @Override
            protected void convert(ViewProvider viewProvider, VideoListMo item, int position) {
                viewProvider.setText(ResourceTable.Id_name_tv, item.getName());
                viewProvider.setText(ResourceTable.Id_content_tv, item.getDescription());
                viewProvider.setImageResource(ResourceTable.Id_image_iv, item.getSourceId());
            }
        };
        ...
        playListContainer.setItemClickedListener(new ListContainer.ItemClickedListener() {
            @Override
    ...
    private void initAdvertisement() {
        ...
        if (advViewPager instanceof PageSlider) {
            ...
            advPageSlider.setClickedListener(component -> {
                startFa();
            });
        } else {
    ...
    private void startFa() {
        ...
        Operation operation = new Intent.OperationBuilder().withBundleName(getBundleName())
                .withAbilityName("com.huawei.codelab.MainAbility").withAction("action.video.play").build();
