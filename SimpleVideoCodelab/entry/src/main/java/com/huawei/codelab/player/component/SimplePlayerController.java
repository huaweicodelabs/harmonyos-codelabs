...
public class SimplePlayerController extends ComponentContainer {
                    ...
                    new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (status == PlayerStatus.PREPARING) {
                                ...
                                playToggle.setPixelMap(ResourceTable.Media_ic_music_stop);
                                playToggle.setClickable(true);
                                progressBar.setEnabled(true);
                            } else if (status == PlayerStatus.PAUSE) {
                                controllerHandler.sendEvent(
                                        Constants.PLAYER_PROGRESS_RUNNING,
                                        EventHandler.Priority.IMMEDIATE);
                                playToggle.setPixelMap(ResourceTable.Media_ic_music_play);
                            } else if (status == PlayerStatus.STOP || status == PlayerStatus.COMPLETE) {
                                controllerHandler.sendEvent(
                                        Constants.PLAYER_PROGRESS_RUNNING,
                                        EventHandler.Priority.IMMEDIATE);
                                playToggle.setPixelMap(ResourceTable.Media_ic_update);
                                progressBar.setEnabled(false);
                            } else {
    ...
    private void initComponent() {
        ...
        if (playerController.findComponentById(ResourceTable.Id_play_controller) instanceof Image) {
            playToggle = (Image) playerController.findComponentById(ResourceTable.Id_play_controller);
        }
    ...
    private void initListener() {
        ...
        playToggle.setClickedListener(new ClickedListener() {
            @Override
            public void onClick(Component component) {
                if (implPlayer.isPlaying()) {
                    implPlayer.pause();
                } else {
                    if (implPlayer.getPlayerStatu() == PlayerStatus.STOP) {
                        implPlayer.replay();
                    } else {
                        implPlayer.resume();
                    }
